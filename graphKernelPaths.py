...
class GraphKernel(Kernel):
    def __init__(self, *, ngram_length, X):
        """
        Args:
            X: A list of strings, where each string corresponds to a document.
            X_prime: (during testing) A list of strings, where each string corresponds to a document.
        ...
        self.graphs = self.convertToGraphs(X)
    ...
    def convertToGraphs(self, X):
        ...
        graphs = dict()
        try:
            result_conll = tweebo_api.parse_conll(text_data) #ISSUE? Only finding 7 graphs!
    ...
    def evaluate(self, s, t):
        """
         dependency graph kernel function evaluation.

        Args:
            s: A string corresponding to a tweet.
            t: A string corresponding to a tweet.

        Returns:
            A float from evaluating K(G(s),G(t))
        """
        max = 0
        try:
            dep_tree_s = self.graphs[s]
            dep_tree_t = self.graphs[t]
