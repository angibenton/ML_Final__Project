#changes made in get_args() func. Removed topics and a problem input to choose data to use
import os
import pickle
import argparse
...
def get_args():
    ...
    parser.add_argument("--problem", type=str, required=True, choices=["offensive-normal", "hate-offensive"], help="Classification problem.")#added
...
def check_args(args):
    mandatory_args = {'datadir', 'mode', 'model_file', 'predictions_file', 'kernel','problem'}
...
def test(args):
    ...
    X, _ = load_data(args.mode.lower(), args.datadir, args.num_articles, args.problem)
...
def train(args):
    ...
    """
    X, y = load_data(args.mode.lower(), args.datadir, args.num_articles, args.problem)
    y = np.array([1 if c == 1 else 0 for c in y]) #change to 0/1 input
